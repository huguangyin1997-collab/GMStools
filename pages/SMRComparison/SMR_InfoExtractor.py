...
class SMR_InfoExtractor:
    ...
    def _get_dir_files(self, directory):
        ...
        for root, dirs, files in os.walk(directory):
            for f in files:
                ...
                elif low == 'vbmetadeviceinfo.deviceinfo.json':
                    categorized["vbmetadeviceinfo"].append(fp)
    ...
    def extract_vbmeta_info(self, directory):
        """从VbmetaDeviceInfo.deviceinfo.json中提取vbmeta分区信息"""
        vbmeta_info = {
            "found": False,
            "images": [],
            "image_names": [],
        ...
        json_files = self._get_dir_files(directory)["vbmetadeviceinfo"]

        if json_files:
            json_file = json_files[0]
            vbmeta_info["file_path"] = json_file
            try:
                data = self.file_utils.read_json_file(json_file)
                if data:
                    vbmeta_info["found"] = True
                    vbmeta_info["raw_data"] = data
                    images = (data.get("images", None)
                              or data.get("partitions", None)
                              or data.get("vbmeta_images", None)
                              or [])
                    vbmeta_info["images"] = images
                    vbmeta_info["image_names"] = [
                        img.get("name", img.get("partition_name", "unknown"))
                        for img in images
                    ]
            except Exception as e:
                print(f"读取VbmetaDeviceInfo文件 {json_file} 时出错: {e}")
